...
a_b1 <- ranef(a)[["sujetos"]][["confidence_key.norm"]]
a_intercept <- fixef(a)[["(Intercept)"]]
a2_b1 <- ranef(a2)[["sujetos"]][["confidence_key.norm"]]
a2_intercept <- ranef(a2)[["sujetos"]][["(Intercept)"]]
...
for (j in 1:length(unique(d$sujetos))) {
  ...
  for (i in 1:length(d_sujeto$confidence_key.norm)) {
    # equation model a
    y.model_a_sujeto[i] <- invlogit(a_intercept +  a_b1[j] * d_sujeto$confidence_key.norm[i])
    
    # equation model a2
    y.model_a2_sujeto[i] <- invlogit(a2_intercept[j] +  a_b1[j] * d_sujeto$confidence_key.norm[i])
  }
...
respuestas     <- matrix( c(resp_correctas_datos,
                    ...
                    resp_incorrectas_modelo_a2), ncol = 3, byrow = TRUE) 

df <- data.frame(fuente = rep(c("datos", "modelo_a", "modelo_a2"), each=2),
                 es_correcta = rep(c("correctas","incorrectas"), times = 3),
                 nro_correctas = ´)

tab <- table(df$fuente, df$es_correcta)
