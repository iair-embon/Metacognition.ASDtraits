...
p2 <- ggplot(df, aes(x=x, fill= factor(y) )) + # group=y, color=y, fill=y
...
```
# Ver como hacer para que represente el modelo mixto
```{r grafico_curva_log_multinivel_un_sujeto, echo=TRUE}

### ver si puedo hacer algo con estas lineas, pueden servir
d_sujeto <-  d[d$sujeto == unique(d$sujetos)[1],]
...
d_sujeto_uni_slope < d_sujeto_uni_slope[y == ]

plot(discrimination_is_correct ~ confidence_key.norm, 
     data=d_sujeto, col="steelblue")
lines(y_predicted_sujeto ~ d_sujeto$confidence_key.norm, 
      lwd=2)
```
